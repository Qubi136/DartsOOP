...
int Target::getScore(int x, int y) {
    ...
    for (const auto& range : this->angles) {
        if (angle >= range.first.first && angle <= range.first.second) {
            if (isSingleBull(x, y, 150) && !isDoubleBull(x, y, 150)) { score = 25; }
            else if (isDoubleBull(x, y, 150)) { score = 50; }
            else if (isDouble(x, y, 150)) { score = range.second * 2; }
            else if (isTreble(x, y, 150)) { score = range.second * 3; }
